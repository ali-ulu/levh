...
def _event_date(m: Memory) -> str:
    ...
    captured into StackMemory), truncated to ``YYYY-MM-DD``. Shared by
...
class MemoryEngine:
    ...
    async def generate_context_file(
        ...
        if style == "cursor":
            ...
            lines.append(f"# Generated by StackMemory on {now}. Do not edit by hand.")
        else:
            lines.append(f"# {title}")
            lines.append("")
            lines.append(f"> Generated by StackMemory on {now}. Do not edit by hand — ")
            lines.append("> update memories in StackMemory and regenerate.")
        lines.append("")
    ...
    async def timeline(self, days: int = 30, project: str | None = None) -> list[dict]:
        ...
        captured into StackMemory) if present. Returns day-groups sorted
        most-recent-first."""
        from datetime import datetime, timedelta, timezone
    ...
    async def restore(self, snapshot: dict, replace: bool = False) -> dict:
        ...
        if not isinstance(snapshot, dict) or snapshot.get("format") != BACKUP_FORMAT:
            raise ValueError("not a StackMemory backup snapshot")
